	...
	$('.editProduct').click(function(){
		...
		$.ajax({
			...
			success : function(data){
				...
				CKEDITOR.instances['demo'].setData(data.product.desciption);
				let html1 = '';
				...
				$.each(data.producttype,function(key,value){
					if(data.product.idProducType == value['id']){
			...
			$.ajax({
				...
				success : function(data){
					...
					if(data.error == 'true')
					{
						...
						if(data.message.desciption)
						{
							$('.errorDesciption').show();
							$('.errorDesciption').text(data.message.desciption[0]);
							$('.desciption').val('');
						}
					}else{
						toastr.success(data.result, 'Thông báo', {timeOut: 5000});
